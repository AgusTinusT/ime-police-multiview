...
const submitFeedbackForm = async () => {
    ...
    try {
        ...
        if (res.ok && data.status === "success") {
            successMessage.value =
                data.message ||
                "Laporan berhasil dikirim langsung ke Discord Dispatcher!";
            feedbackForm.value = {
...
<template>
    <TacticalLayout>
        ...
        <!-- Main Content Portal -->
        <main
            class="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 flex flex-col gap-6"
        >
            <!-- Hero Title -->
            <div class="space-y-2 text-center sm:text-left">
                <div
                    class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-sky-950/80 border border-sky-500/40 text-sky-300 font-mono text-xs font-bold"
                >
                    <img
                        :src="iconRadio"
                ...
                <h1
                    class="text-2xl sm:text-3xl font-black text-slate-100 tracking-tight"
                >
                    Portal Laporan & Usulan Streamer
                </h1>
                <p class="text-xs sm:text-sm text-slate-400 leading-relaxed">
                    Ajukan streamer polisi baru, laporkan perubahan
                    pangkat/callsign dinas, atau sampaikan masukan langsung ke
            ...
            <div
                v-if="errorMessage"
                class="bg-red-950/90 border border-red-500/50 rounded-2xl p-4 text-xs text-red-300 font-mono flex items-start gap-3 shadow-lg"
            >
                <span class="text-lg leading-none">✕</span>
                <div>
                    <div class="font-bold text-red-200 mb-0.5">
            ...
            <form
                ...
                class="bg-slate-900/90 border border-slate-800 rounded-2xl p-5 sm:p-7 shadow-2xl space-y-5"
            >
                <!-- Category Selector -->
                <div>
                    <label
                        class="text-xs font-bold uppercase tracking-wider text-slate-400 font-mono block mb-2"
                        >Pilih Kategori Laporan *</label
                    >
                    <div class="grid grid-cols-2 sm:grid-cols-4 gap-2">
                        ...
                        >
                            ...
                            <div>
                                ...
                                <div
                                    class="text-[10px] text-slate-300/80 font-normal mt-0.5"
                                >
                                    Streamer polisi baru
                                </div>
                            </div>
                        </button>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label
                            ...
                            >Nama Pengirim / IC Name (Opsional)</label
                        >
                        <input
                    ...
                    <div>
                        <label
                            class="text-xs font-semibold text-slate-300 block mb-1"
                            >Handle / Link YouTube Streamer</label
                        >
                        <input
                ...
                >
                    <div class="sm:col-span-2">
                        <label
                            class="text-xs font-semibold text-slate-300 block mb-1"
                            >Nama Karakter Polisi / Callsign</label
                        >
                        <input
                ...
                <div>
                    <label
                        class="text-xs font-semibold text-slate-300 block mb-1"
                        >Detail Pesan / Catatan Laporan *</label
                    >
                    <textarea
                        ...
                        :placeholder="
                            feedbackForm.type === 'CHANNEL_REQUEST'
                                ? 'Jelaskan jadwal live rutin streamer atau informasi pendukung lainnya...'
                                : feedbackForm.type === 'DATA_CORRECTION'
                                  ? 'Jelaskan data apa yang perlu diperbarui (misal kenaikan pangkat jadi Sergeant, perbaikan callsign)...'
                                  : 'Tuliskan detail masukan atau kendala teknis yang Anda temui...'
                        "
                ...
                >
                    <span
                        class="text-[11px] text-slate-500 font-mono hidden sm:inline"
                        >Terkirim langsung ke Discord Dispatcher</span
                    >
